...
console.log(response);			
			...
			for(i = 0; i < response.companies.length; i++) {
				tblSrc += "<tr>";
				tblSrc += "<td>" + response.companies[i].id + "</td>";
				tblSrc += "<td>" + (response.companies[i].name || "") + "</td>";

				tblSrc += "<td>" + (response.companies[i].description || "") + "</td>";
				tblSrc += "<td>" + (response.companies[i].last_modified || "") + "</td>";
				tblSrc += "<td>";
				tblSrc += "<a href=\"/company/" + response.companies[i].id + "\" class=\"material-icons\">mode_edit</a>";
				tblSrc += "<a href=\"javascript:Companies.remove(" + response.companies[i].id + ");\" class=\"material-icons text-danger\">clear</a>";
				tblSrc += "</td>";
				tblSrc += "</tr>";
			}

			$("#tblCompanies tbody").html(tblSrc);

			var minPage = Math.max(1, parseInt(response.page) - 2);
			var maxPage = Math.min(response.pages, parseInt(response.page) + 2);
			for(i = minPage; i <= maxPage; i++) {
				ulSrc += "<li class=\"page-item " + (i == response.page ? "active" : "") + "\"><a href=\"javascript:Companies.getAll({page:" + i + "})\" class=\"page-link\">" + i + "</a></li>";
			}
			ulSrc += "<li class=\"page-item\"><a href=\"javascript:Companies.getAll({page:'all'});\" class=\"page-link\">Show All</a></li>";
